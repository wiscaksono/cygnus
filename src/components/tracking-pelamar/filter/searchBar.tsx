...
export const SearchBar = ({ filter, setFilter }: FilterTrackingPelamarProps) => {
  ...
  return (
    <div className="relative flex w-full flex-1 items-center">
      ...
      <input
        ...
        onKeyDown={(e) => e.key === "Enter" && setFilter({ ...filter, name: e.currentTarget.value })}
      />
      <div className="absolute inset-y-0 right-0 flex py-1.5 pr-1.5">
        <kbd className="inline-flex items-center rounded border border-gray-200 px-1 font-sans text-xs text-gray-400">Ctrl + F</kbd>
      </div>
    </div>
  );
};
